...
class Series
  ...
  def slices(length_of_slice)
    raise ArgumentError if length_of_slice > @string.size

    result = []
    array_nums = @string.chars.map(&:to_i)
    array_nums.each_with_index do |_, index|
      break if index + length_of_slice > array_nums.size
      up_to = index + length_of_slice
      result << array_nums[index...up_to]
    end
    result
  end
end
